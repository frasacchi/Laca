function out = createStreamwiseVariantv2(obj,varargin)
    ...
    Ignore(Ignore<0) = size(RefDirsLE,2)+Ignore(Ignore<0);
    % for each substantial change in LE and TE direction add a ref line
    for i = setdiff(1:size(RefDirsLE,2)-1,Ignore)
        %check LE
        ...
        if abs(angle)>p.Results.SweepTol
            ...
            normal = cross(old_LE(:,i)-old_LE(:,i+1),old_LE(:,i+1)-old_TE(:,i+1));
            normal = cross(normal,p.Results.Dir);
    %         normal = RefDirsLE(:,i+1).*(1-p.Results.Dir);
            %            normal = normal ./ norm(normal);
            %            normal = cross(p.Results.Dir,[0 0 1]');
    ...
    for i = 1:size(Nodes,2)
        ...
        Panel_normal = cross(p.Results.Dir(:),NodeDir);
        plane_normal = cross(Panel_normal,p.Results.Dir(:));
        ...
        new_LE(:,i) = dot(tmp_LE(:,i)-Nodes(:,i),p.Results.Dir(:))*p.Results.Dir(:) + Nodes(:,i);
        new_TE(:,i) = dot(tmp_TE(:,i)-Nodes(:,i),p.Results.Dir(:))*p.Results.Dir(:) + Nodes(:,i);
        
        % correct chordlength (large AoAs shorthen the effective chord length)
        new_LE(:,i) = new_LE(:,i) + tand(abs(Normalwash(i))/2)*sind(Normalwash(i))*norm(vec)*-p.Results.Dir(:);
        new_TE(:,i) = new_TE(:,i) + tand(abs(Normalwash(i))/2)*sind(Normalwash(i))*norm(vec)*p.Results.Dir(:);
    end
